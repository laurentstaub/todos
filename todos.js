...
// one parameter is passed to `use`, the callback executed for every request
...
// session middleware stores data in the session in the form of a cookie
app.use(session({
  cookie: {
    httpOnly: true,
...
app.get("/lists",
  // the route can use several callbacks, but it requires next() to do so
  ...
  catchError(async (req, res) => {
    ...
    // render is a method renders a template and sends the rendered HTML to the client
    res.render("lists", {
      todoLists,
      todosInfo,
    });
...
app.post("/lists/:todoListId/complete_all",
  ...
  catchError(async (req, res) => {
    ...
    if (!allToggled) throw new Error("Not found.");
